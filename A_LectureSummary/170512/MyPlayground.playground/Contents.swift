...
//2. else는 안써도 무방해서 지울 수 있다. 하지만 거짓일 경우의 조건을 입력하지 않은 것이므로, 거짓일 경우에는 return이 되지 않는다. 따라서 if문 바깥, func의 내부에 return false를 별도 입력해주어야 한다.
...
isAble(creditCard: myCreditCard)
